...
- **`internal/nostr/`** — Nostr protocol handling:
  ...
  - `relay.go` — `RelayPool`: subscribes to a single author's events (kinds 0,1,3,5,6,7,9735) with author filter. Auto-reconnects with 5s backoff. `Publisher`: publishes events to write relays (uses a shared lazy-initialized pool via `sync.Once`).
