...
- **`internal/bsky/`** — Bluesky (AT Protocol) bridge (optional):
  ...
  - `poster.go` — `Poster`: outbound bridge. Handles kind-1 (post), kind-5 (delete), kind-6 (repost), kind-7 "+" (like). Guards against double-bridging via `GetAPIDForObject`. Stores AT URI ↔ Nostr event ID mappings. Outbound image support via imeta tag extraction and blob upload (max 4 images, 1MB each). Quote post embeds via q-tag → `embed.record` (or `recordWithMedia` when combined with images). Content warning support via NIP-36 → Bluesky self-labels. Mention facets for `nostr:npub` references in post text (best-effort DID resolution).
